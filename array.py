def k_diff_pairs_in_an_array_532(nums, k):
    ...
    for i in range(len(nums)):
        lookup[nums[i]-k] = (i, nums[i])
    
    #use lookup
    for j in range(len(nums)):
        if nums[j] in lookup and lookup[nums[j]][0] != j:
            pairs.add((lookup[nums[j]][1], nums[j]))
...
def k_diff_pairs_in_an_array_532(nums, k):
    ...
    # lookup construction
    for i in range(len(nums)):
        lookup[nums[i] - k] = (i, nums[i])  # diff :(index, num)
